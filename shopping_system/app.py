...
class ShoppingSystem:
    ...
    def register(self, username, password, email, user_type, shop_name="", contact=""):
        """用户注册"""
        if username in self.db.data["users"] or username in self.db.data["merchants"] or username in self.db.data[
            "admins"]:
            return False
    ...
    def get_all_users(self):
        ...
        # 添加商家
        for username, merchant_data in self.db.data["merchants"].items():
            users.append({
                "username": username,
                "email": merchant_data.get("contact", ""),
                "type": "merchant"
            })

        return users

    def delete_user(self, username, user_type):
        """删除用户"""
        if user_type == "user":
            if username in self.db.data["users"]:
            ...
            if username in self.db.data["merchants"]:
                # 同时删除该商家的所有商品
                merchant_products = [pid for pid, pdata in self.db.data["products"].items()
                                     if pdata["merchant"] == username]
                for pid in merchant_products:
                    if pid in self.db.data["products"]:
                        del self.db.data["products"][pid]
                del self.db.data["merchants"][username]
                self.db.save_data()
                return True
        return False
    ...
    def remove_product(self, product_id):
        ...
        if product_id in self.db.data["products"]:
            del self.db.data["products"][product_id]
            self.db.save_data()
            return True
        return False
